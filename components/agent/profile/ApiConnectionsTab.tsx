...
export function ApiConnectionsTab({ agentId, isDark }: { agentId: string; isDark: boolean }) {
            ...
            {connections.length > 0 ? (
                        ...
                        >
                            ...
                            {conn.endpoints?.length > 0 && (
                                <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                                    <div className={cn('text-xs font-medium mb-2', isDark ? 'text-zinc-400' : 'text-zinc-500')}>
                                        사용 가능한 기능
                                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <button
                            onClick={() => {
                                setSelectedPreset({ id: 'custom' } as ApiPreset)
                                setShowAddModal(true)
                            }}
                            className={cn(
                    ...
                    >
                        ...
                        {!selectedPreset ? (
                            <div className="space-y-6">
                                ...
                                {Object.entries(presetsByCategory).map(([category, categoryPresets]) => (
                                    <div key={category}>
                                        <h4 className={cn('text-sm font-medium mb-3', isDark ? 'text-zinc-300' : 'text-zinc-700')}>
                                            {apiCategoryLabels[category] || category}
                                        </h4>
                                        <div className="grid grid-cols-2 gap-3">
                                            {categoryPresets.map((preset) => (
                                                ...
                                                >
                                                    <div className={cn('font-medium mb-1', isDark ? 'text-white' : 'text-zinc-900')}>
                                                        {preset.name}
                                                    </div>
                                ...
                                <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800">
                                    ...
                                    >
                                        <Plus className={cn('w-6 h-6 mx-auto mb-2', isDark ? 'text-zinc-500' : 'text-zinc-400')} />
                                        <div className={cn('font-medium', isDark ? 'text-white' : 'text-zinc-900')}>
                                            커스텀 API 추가
                                        </div>
                                        <div className={cn('text-xs mt-1', isDark ? 'text-zinc-400' : 'text-zinc-600')}>
                                            직접 API 정보를 입력합니다
                                        </div>
                                    </button>
                                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <button
                                    onClick={resetForm}
                                    className={cn('flex items-center gap-2 text-sm', isDark ? 'text-zinc-400' : 'text-zinc-600')}
                                >
                                ...
                                {selectedPreset.id !== 'custom' && (
                                    <div className={cn('p-4 rounded-xl', isDark ? 'bg-zinc-800' : 'bg-zinc-50')}>
                                        <div className={cn('font-medium mb-2', isDark ? 'text-white' : 'text-zinc-900')}>
                                            {selectedPreset.name}
                                        </div>
                                        {selectedPreset.setup_guide && (
                                            <div className={cn('text-sm whitespace-pre-line', isDark ? 'text-zinc-400' : 'text-zinc-600')}>
                                                {selectedPreset.setup_guide}
                                            </div>
                                        )}
